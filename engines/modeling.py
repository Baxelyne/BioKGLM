...
CONFIG_NAME = 'bioLM_config.json'
WEIGHTS_NAME = 'post_trained_bioLM_model.bin'
...
class BertLayerFuse(nn.Module):
    def __init__(self, config):
        super(BertLayerFuse, self).__init__()
...
class BertLayerAfter(nn.Module):
    def __init__(self, config):
        super(BertLayerAfter, self).__init__()
...
class BertLayerBefore(nn.Module):
    def __init__(self, config):
        super(BertLayerBefore, self).__init__()
        self.attention = BertAttention_simple(config)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        layer_after = BertLayerAfter(config)
        layer_before = BertLayerBefore(config)
        layer_fuse = BertLayerFuse(config)
        layers = []
        for t in config.layer_types:
            if t == "before":
                layers.append(copy.deepcopy(layer_before))
            if t == "after":
                layers.append(copy.deepcopy(layer_after))
            if t == "fuse":
                layers.append(copy.deepcopy(layer_fuse))
...
class BertPostTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(BertPostTrainingHeads, self).__init__()
        self.predictions = BertLMPredictionHead(config, bert_model_embedding_weights)
...
class PostTrainedBertModel(nn.Module):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained models.
    """

    def __init__(self, config, *inputs, **kwargs):
        super(PostTrainedBertModel, self).__init__()
...
class BertModel(PostTrainedBertModel):
...
class BertForPostTraining(PostTrainedBertModel):
    ...
    def __init__(self, config):
        ...
        self.cls = BertPostTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        prediction_scores, seq_relationship_score, prediction_scores_ent = self.cls(sequence_output, pooled_output,
                                                                                    candidate)
...
class BertForMaskedLM(PostTrainedBertModel):
    ...
    def forward(self, input_ids, input_ents, ent_mask=None, token_type_ids=None, attention_mask=None,
...
class BertForNextSentencePrediction(PostTrainedBertModel):
...
class BertForEntityTyping(PostTrainedBertModel):
...
class BertForSTSB(PostTrainedBertModel):
    def __init__(self, config, num_labels=2):
...
class BertForSequenceClassification(PostTrainedBertModel):
...
class BertForNQ(PostTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None,
        ...
        _, pooled_output = self.bert(flat_input_ids, flat_token_type_ids, flat_attention_mask, flat_input_ent,
                                     flat_ent_mask, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
...
class BertForMultipleChoice(PostTrainedBertModel):
...
class BertForTokenClassification(PostTrainedBertModel):
...
class BertForQuestionAnswering(PostTrainedBertModel):
