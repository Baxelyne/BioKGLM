...
from engines.typing import BertTokenizer as BertTokenizer_label
from engines.tokenization import BertTokenizer
from engines.modeling import BertForEntityTyping
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer_label, tokenizer, threshold):
    ...
    entity2id = {}
    with open("bio_kg_embed/entity2id.txt") as fin:
        fin.readline()
        for line in fin:
...
def main():
    ...
    parser.add_argument("--bioLM_model", default=None, type=str, required=True,
                        help="bio post-trained LM model")
    ...
    tokenizer_label = BertTokenizer_label.from_pretrained(args.bioLM_model, do_lower_case=args.do_lower_case)
    tokenizer = BertTokenizer.from_pretrained(args.bioLM_model, do_lower_case=args.do_lower_case)
    ...
    # Prepare model
    model, _ = BertForEntityTyping.from_pretrained(args.bioLM_model,
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_input_ent, all_ent_mask,
                                   all_labels)
        if args.local_rank == -1:
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if global_step % 150 == 0 and global_step > 0:
                        ...
                        output_model_file = os.path.join(args.output_dir, "post_trained_bioLM_model.bin_{}".format(global_step))
                        torch.save(model_to_save.state_dict(), output_model_file)
            model_to_save = model.module if hasattr(model, 'module') else model
            output_model_file = os.path.join(args.output_dir, "post_trained_bioLM_model.bin_{}".format(epoch))
            torch.save(model_to_save.state_dict(), output_model_file)
    exit(0)
